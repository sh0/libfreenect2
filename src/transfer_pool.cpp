...
void TransferPool::onTransferCompleteStatic(libusb_transfer* transfer)
{
    ...
    uint64_t timestamp_usb = std::chrono::duration_cast<std::chrono::microseconds>(
      std::chrono::high_resolution_clock::now().time_since_epoch()
    ).count() / 100;
  #else
    uint64_t timestamp_usb = 0;
  #endif

  TransferPool::Transfer *t = reinterpret_cast<TransferPool::Transfer*>(transfer->user_data);
  t->pool->onTransferComplete(t, timestamp_usb);
}

void TransferPool::onTransferComplete(TransferPool::Transfer* t, uint64_t timestamp_usb)
{
  ...
  // process data
  processTransfer(t->transfer, timestamp_usb);
...
void BulkTransferPool::processTransfer(libusb_transfer* transfer, uint64_t timestamp_usb)
{
  if(transfer->status != LIBUSB_TRANSFER_COMPLETED) return;

  if(callback_)
    callback_->onDataReceived(transfer->buffer, transfer->actual_length, timestamp_usb);
}
...
void IsoTransferPool::processTransfer(libusb_transfer* transfer, uint64_t timestamp_usb)
{
  ...
  for(size_t i = 0; i < num_packets_; ++i)
  {
    ...
    if(callback_)
      callback_->onDataReceived(ptr, transfer->iso_packet_desc[i].actual_length, timestamp_usb);
